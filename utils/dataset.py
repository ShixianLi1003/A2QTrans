...
class Generic_BE_DS():
    ...
    def generic_preprocess(self, file_list, file_list_full, shape_hw_list, data_len, train_test_list=None):
        ...
        if self.aplly_BE:
            if self.full_ds:
                mask_u2n_list, x_u2n_list, y_u2n_list, h_u2n_list, w_u2n_list = mask_hw(
                    full_ds=self.full_ds, 
                    img_path=file_list_full, 
                    shape_hw=shape_hw_list, 
                    # save_path=f".//Nabirds_process_data/Nabirds_{self.mode}"
                    save_path=f"./CUB_process_data/CUB_{self.mode}"
                    # save_path=f"./dogs_process_data/dogs_{self.mode}"
                )
        ...
        for ind, file in enumerate(file_list[:data_len]):
            ...
            img_temp = imageio.imread(os.path.join(self.root, self.base_folder, file))
            ...
            if self.aplly_BE and self.full_ds:
            ...
            if self.aplly_BE or self.gt_bbox:
                if self.padding:
        ...
        if self.aplly_BE:
            if self.full_ds:
                mask_u2n_list, x_u2n_list, y_u2n_list, h_u2n_list, w_u2n_list = mask_hw(
                    ...
                    shape_hw=shape_hw_list, 
                    # ./Nabirds_{self.mode}"
                    save_path=f"./CUB_{self.mode}"
                    # save_path=f"./dogs_{self.mode}"
                )
            else: # for debug
                img_path = os.path.join(self.root, self.base_folder, file_list)
                img_temp = imageio.imread(img_path)
    ...
    def generic_getitem_lowMem(self, index):
        ...
        img_temp = imageio.imread(os.path.join(self.root, self.base_folder, file_temp))
...
class CUB(Generic_BE_DS):

    def __init__(self, ds_name, root, is_train=True, data_len=None, transform=None, aplly_BE=True, low_memory=True, img_size=400):
        ...
        self.aplly_BE = aplly_BE
        self.low_memory = low_memory

        if (self.aplly_BE + self.gt_bbox + self.gt_parts) > 1 :
            raise Exception("Only one cropping mode (aplly_BE, bbox, parts) can be chosen")
        ...
        if self.aplly_BE:
            ...
            for img_name in self.file_list:
                img_temp = Image.open(os.path.join(self.root, self.base_folder, img_name))
                # img_temp = imageio.imread(os.path.join(self.root, self.base_folder, img_name))
                img_temp = Image.open(os.path.join(self.root, self.base_folder, img_name))

                width, height = img_temp.size

                shape_hw_temp = [height, width] 
            ...
            self.img = \
                [imageio.imread(os.path.join(self.root, self.base_folder, file)) \
...
class dogs(Generic_BE_DS): #(Dataset):
    ...
    def __init__(self,
        ...
        if self.aplly_BE:
            ...
            for image_name, target_class in self._flat_breed_images:
                ...
                img_temp = imageio.imread(os.path.join(img_name))
                shape_hw_temp = [img_temp.shape[0], img_temp.shape[1]] # h_max (y), w_max (x)
...
class NABirds(Generic_BE_DS): #(Dataset):
    ...
    def __init__(self, ds_name, root, is_train=True, data_len=None, transform=None, aplly_BE=True, low_memory=True, img_size=448):
        ...
        if self.aplly_BE:
            ...
            for sample in self.data.iloc:
                ...
                img_temp = imageio.imread(os.path.join(img_name_full))
                shape_hw_temp = [img_temp.shape[0], img_temp.shape[1]] # h_max (y), w_max (x)
