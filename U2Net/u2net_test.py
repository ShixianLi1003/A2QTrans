...
def mask_hw(full_ds=True, img_path=None, shape_hw=None, save_path=None):

    if save_path and all(os.path.exists(f"{save_path}_{suffix}.npy") 
                         for suffix in ['mask_out', 'start_x', 'start_y', 'h', 'w']):
        print(f"Loading saved mask data from {save_path}")
        mask_out_np_list = np.load(f"{save_path}_mask_out.npy", allow_pickle=True)
        start_x_list = np.load(f"{save_path}_start_x.npy", allow_pickle=True)
        start_y_list = np.load(f"{save_path}_start_y.npy", allow_pickle=True)
        h_list = np.load(f"{save_path}_h.npy", allow_pickle=True)
        w_list = np.load(f"{save_path}_w.npy", allow_pickle=True)
    ...
    for i_test, data_test in enumerate(test_salobj_dataloader):
        ...
        transform_mask = transforms.Compose([
            ...
            transforms.Resize((shape_hw_i[0], shape_hw_i[1]), Image.BILINEAR), 
            transforms.ToTensor(),
            ])
    ...
    if save_path:
        print(f"Saving mask data to {save_path}")
        np.save(f"{save_path}_mask_out.npy", mask_out_np_list)
        np.save(f"{save_path}_start_x.npy", start_x_list)
        np.save(f"{save_path}_start_y.npy", start_y_list)
        np.save(f"{save_path}_h.npy", h_list)
        np.save(f"{save_path}_w.npy", w_list)
